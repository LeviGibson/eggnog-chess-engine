  ...
  */
  ...
  function update_board() {
    ...
    for (var row = 0; row < 8; row++) {
      ...
      for (var col = 0; col < 16; col++) {
        ...
        if ((square & 0x88) == 0)
          // draw pieces
          document.getElementById(square).innerHTML = '<img name="piece" style="width: ' + 
                                                       (width ? width / 8: 400 / 8) + "px; " + (playerSide ? "-webkit-transform:rotate(180deg);":"") +
                                                      '" draggable="true" id="' + 
                                                       board[square] + '" src ="chessboard/src/Images/' + 
                                                      (board[square]) +'.gif">';
      }
    }
  }
  ...
  function drag_over(event, square) {        
    ...
    if (square == user_source)
      event.target.src = 'chessboard/src/Images/0.gif';
  }
...
function executeAsync(func) {
  ...
  return {    
    ...
    flip_board: function() { flip_board() },

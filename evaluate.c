...
int evaluate(){
    ...
    for (int piece = P; piece <= K; piece++){
        ...
        for (int _ = 0; _ < bb_count; _++) {
            ...
            if (piece == B){
                eval += count_bits(get_bishop_attacks(target, occupancies[white] | bitboards[p])) * 9;
            } else if (piece == R){
                eval += count_bits(get_rook_attacks(target, occupancies[white] | bitboards[p])) * 7;
            }
    ...
    for (int piece = p; piece <= k; piece++){
        ...
        for (int _ = 0; _ < bb_count; _++){
            ...
            if (piece == b){
                eval -= count_bits(get_bishop_attacks(target, occupancies[black] | bitboards[P])) * 9;
            } else if (piece == r){
                eval -= count_bits(get_rook_attacks(target, occupancies[black] | bitboards[P])) * 7;
            }
